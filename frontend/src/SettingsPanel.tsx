...
import { getModelSettings, getProjectConfig, MULTI_AGENT_TURN_LIMITS, saveModelSettings, saveProjectConfig, type AgentRoleName, type AgentWorkflow, type ModelSettings, type ProjectConfig } from './api'
...
export function SettingsPanel({ sessionId, crossSessionMemory, onCrossSessionMemoryChange, agentWorkflow, onAgentWorkflowChange, busy }: SettingsPanelProps) {
  ...
  const saveWorkflow = async () => {
    ...
    const simplifiedWorkflow: AgentWorkflow = {
      ...workflowDraft,
      agent_config:Object.fromEntries((Object.keys(workflowDraft.agent_config) as AgentRoleName[]).map((role) => [role, {
        ...workflowDraft.agent_config[role],
        enabled:true,
        max_turns:role === 'single' ? workflowDraft.agent_config[role].max_turns : MULTI_AGENT_TURN_LIMITS[role],
      }])) as AgentWorkflow['agent_config'],
    }
    try { await onAgentWorkflowChange(simplifiedWorkflow); setWorkflowDraft(simplifiedWorkflow); setNotice(t('settings.workflowSaved')) }
  ...
  const savedModeHelp = t(agentWorkflow.agent_mode === 'single' ? 'settings.singleComparison' : agentWorkflow.agent_mode === 'adaptive' ? 'settings.adaptiveReason' : 'settings.workflowReason')
  const navigation = [
  ...
  return <div className="settings-panel">
    ...
    <section className="settings-detail">
      ...
      {section === 'agents' && <section className="settings-card workflow-card"><div className="workflow-current"><CircleCheck size={18}/><div><small>{t('memory.enabled')}</small><strong>{t(`settings.mode.${agentWorkflow.agent_mode}`)}</strong><span>{savedModeHelp}</span></div></div><div className="workflow-mode" role="group" aria-label={t('settings.workflowMode')}>{(['multi','single','adaptive'] as const).map((mode) => <button type="button" className={workflowDraft.agent_mode === mode ? 'active' : ''} aria-pressed={workflowDraft.agent_mode === mode} onClick={() => setWorkflowDraft({ ...workflowDraft, agent_mode:mode })} key={mode}>{workflowDraft.agent_mode === mode && <CircleCheck className="mode-check" size={13}/>}<strong>{t(`settings.mode.${mode}`)}</strong><small>{t(`settings.mode.${mode}Detail`)}</small></button>)}</div><p className="config-help"><ShieldCheck size={13}/>{t(workflowDraft.agent_mode === 'single' ? 'settings.singleComparison' : workflowDraft.agent_mode === 'adaptive' ? 'settings.adaptiveReason' : 'settings.workflowReason')}</p><div className="workflow-roles">{visibleRoles.map((role, index) => { const config = workflowDraft.agent_config[role]; const fixedTurns = role === 'single' ? null : MULTI_AGENT_TURN_LIMITS[role]; return <details className={`workflow-role ${role}`} key={role} open={workflowDraft.agent_mode === 'single'}><summary><span>{workflowDraft.agent_mode === 'single' ? <SlidersHorizontal size={13}/> : index + 1}</span><div><strong>{t(`roles.${role}`)}</strong><small>{t(`roleGoals.${role}`)}</small></div></summary><div className="role-options"><label><span>{t('settings.roleTurns')}</span>{fixedTurns === null ? <input type="number" min="1" max="30" value={config.max_turns} onChange={(event) => updateRole(role, { max_turns:Math.max(1, Math.min(30, Number(event.target.value) || 1)) })}/> : <output>{fixedTurns} · {t('settings.roleTurns')}</output>}</label><label><span>{t('settings.roleInstruction')}</span><textarea maxLength={2000} value={config.instruction} placeholder={t('settings.roleInstructionPlaceholder')} onChange={(event) => updateRole(role, { instruction:event.target.value })}/></label></div></details> })}</div><button className="settings-save" type="button" disabled={savingWorkflow || busy} onClick={() => void saveWorkflow()}><Save size={13}/>{t(savingWorkflow ? 'settings.saving' : 'settings.saveWorkflow')}</button></section>}
