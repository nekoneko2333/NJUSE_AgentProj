...
class SQLiteStore:
    ...
    def create_session(self, *, task: str, workspace: str, locale: str, command_mode: str = "auto") -> tuple[SessionSnapshot, ConversationTurn]:
        now, session_id, turn_id = utc_now(), str(uuid4()), str(uuid4())
        title = task.strip().replace("\n", " ")[:48]
        with self._connection() as connection:
            connection.execute(
                "INSERT INTO sessions(id,title,task,workspace,locale,status,command_mode,created_at,updated_at) VALUES(?,?,?,?,?,'created',?,?,?)",
                (session_id, title, task, workspace, locale, command_mode, now, now),
            )
            connection.execute(
    ...
    def get_session(self, session_id: str) -> SessionSnapshot | None:
        ...
        if row is None:
            return None
        return SessionSnapshot(**dict(row), turns=self.list_turns(session_id), events=self.list_events(session_id))

    def list_sessions(self, limit: int = 40) -> list[SessionListItem]:
        with self._connection() as connection:
            rows = connection.execute(
                "SELECT s.id,s.title,s.task,s.workspace,s.locale,s.status,s.updated_at,s.command_mode,COUNT(t.id) AS turn_count FROM sessions s LEFT JOIN turns t ON t.session_id=s.id GROUP BY s.id ORDER BY s.updated_at DESC LIMIT ?",
                (limit,),
            ).fetchall()
