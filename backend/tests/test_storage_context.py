...
class StorageContextTests(unittest.TestCase):
    def test_execution_state_survives_reopen_and_supports_retry(self):
        with TemporaryDirectory() as directory:
            ...
            second = store.append_turn(session.id, "second", "zh-CN")
            store.update_execution_turn(execution.id, second.id)
            self.assertEqual(store.get_execution(execution.id).turn_id, second.id)
